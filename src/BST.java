...
class BSTNode {
	...
	public boolean isLeaf() {
		return (left == null || left.root == null) && (right == null || right.root == null);
	}
	...
	public String toString() {
		...
		if (left == null || left.root == null) str += "(null)";
		else str += "(" + left.root.getData() + ")";

		str += data;

		if (right == null || right.root == null) str += "(null)";
...
public class BST {
	 ...
	 *
	 * @param e the integer we want to insert
	 * @return a bool depending if we inserted it or not.
	 */
	public boolean insert(int e) {
		...
		if (this.root != null) {
			if (this.root.getData() >= e) {
				return this.getLeftSubtree().insert(e);

			}
			if (this.root.getData() < e) {
				return this.getRightSubtree().insert(e);
			}
		}
		return false;
	}
	 ...
	 * remove
	 *
	 * @param e the integer to search and remove.
	 * @return a bool depending if we removed something.
	 */
	public boolean remove(int e) {
		...
		if (root.getData() == e) {
			if (getLeftSubtree().root == null && getRightSubtree().root == null) {
				...
				int maxDataLeft =
				    this.getLeftSubtree().removeMax();
				this.root.setData(maxDataLeft);
			}
			return true;
		}
		if (root != null) {
	...
	/**
	 * remove max
	 * Removes the max integer in the subtree
	 *
	 * @return the max integer we removed
	 */
	private int removeMax() {
		BST RT = this.getRightSubtree();
		int maxData;
		if (RT.root == null) {
			maxData = this.root.getData();
	 ...
	 * Search Recursion
	 *
	 * @param e The int we are looking for
	 * @return The Node of the item we are looking for.
	 */
	...
	/**
	 * Search Non Recursion
	 *
	 * @param e integer to search for
	 * @return The Node of the item we are looking for.
	 */
	private BSTNode searchNonRecursion(int e) {
		BST cursor = this;
		do {
			if (cursor.root.getData() == e) {
				return cursor.root;
			}
			if (e < cursor.root.getData()) {
				cursor = cursor.getLeftSubtree();
	...
	/**
	 * Returns the sum of all items in the tree
	 *
	 * @return sum of all the ints in the tree.
	 */
	public int sum() {
		if (root == null) {
			return 0;
...
//		//test 4: remove method
	...
	private static void test2() {
		BST tree = new BST();
		System.out.println("Ziad's Test Cases");
		System.out.println("Initial tree:");
		tree.inOrderPrt(); //test inOrder traversal

		boolean rc;
		//Just added some extra tests for null cases.
		tree.remove(1);
		tree.removeMax();
		tree.sum();
		tree.searchRecursion(1);
		tree.searchNonRecursion(1);
